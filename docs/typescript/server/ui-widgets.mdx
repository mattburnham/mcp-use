...
| --------------------- | ----------------------- | ---------------------------------- | ------------------------------ |
| **`type: "mcpApps"`** | Both MCP Apps + ChatGPT | ✅ ChatGPT<br/>✅ MCP Apps clients | You want maximum compatibility |
| **`type: "appsSdk"`** | ChatGPT Apps SDK only   | ✅ ChatGPT<br/>❌ MCP Apps clients | ChatGPT-only features needed   |

<Note>
**Recommended**: Use `type: "mcpApps"` for new projects to support both ChatGPT and MCP Apps clients. Your widget code remains identical—only the registration differs.
...
See [MCP Apps](/typescript/server/mcp-apps) for complete dual-protocol documentation and [Content Security Policy](./content-security-policy) for CSP configuration.
...
- `openai/widgetCSP`: [Content Security Policy](./content-security-policy) with default trusted domains
...
**Note**: Your CSP domains are merged with your server's base URL automatically. For ChatGPT, OpenAI's required domains (`*.oaistatic.com`, etc.) are also added. For MCP Apps clients, only the domains you declare are used. See [Content Security Policy](./content-security-policy) for full CSP documentation.
...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| **McpUseProvider** | Unified provider that combines all common React setup (StrictMode, ThemeProvider, WidgetControls, ErrorBoundary) | [McpUseProvider →](./widget-components/mcpuseprovider) |
...
When the host supports it (MCP Apps, MCP Inspector), you can also show **streaming** tool arguments in real time via `partialToolInput` and `isStreaming` from `useWidget` for live previews while the LLM generates the tool call. See [Streaming tool arguments](/typescript/server/widget-components/usewidget#streaming-tool-arguments) in the useWidget docs.
...
**Alternative CSP Configuration**: Instead of using the global `CSP_URLS` environment variable, you can configure CSP per-widget in your widget's `appsSdkMetadata['openai/widgetCSP']` (see [Apps SDK Metadata](#apps-sdk-metadata) or [Content Security Policy](./content-security-policy)).
...
See [Debugging Widgets](/inspector/debugging-chatgpt-apps) for complete testing guide.
